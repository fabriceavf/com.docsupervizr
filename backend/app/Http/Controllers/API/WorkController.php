...
// use App\Repository\prod\WorksRepository;
use App\Models\Work;
use App\Http\AgGrid;
...
* @param App\Repository\prod\WorksRepository $WorksRepository
* @param int $id
*/
...
if(
class_exists('\App\Http\Extras\WorkExtras') &&
method_exists('\App\Http\Extras\WorkExtras', 'getRelationsApplyWhenUserCallMultipleData')
){
try{
$relationsWhenDataIsMutlipleHide=\App\Http\Extras\WorkExtras::getRelationsApplyWhenUserCallMultipleData($request);
}catch (\Throwable) {
$relationsWhenDataIsMutlipleHide=[];
}
}
...
if(
class_exists('\App\Http\Extras\WorkExtras') &&
method_exists('\App\Http\Extras\WorkExtras', 'filterAgGridQuery')
){
\App\Http\Extras\WorkExtras::filterAgGridQuery($request,$query);
}


$agGrid=new AgGrid('works',$query);
$data= $agGrid->getData($request);

if(
class_exists('\App\Http\Extras\WorkExtras') &&
method_exists('\App\Http\Extras\WorkExtras', 'AgGridUpdateDataBeforeReturnToUser')
){
$_d=$data['rowData'];
$_d= \App\Http\Extras\WorkExtras::AgGridUpdateDataBeforeReturnToUser($request,$_d);
$data['rowData'] = $_d;
...
return response()->json(Work::count());
}
$data = QueryBuilder::for(Work::class)
->allowedFilters([
...
public function create(Request $request, Work $Works)
{


try{
$can=\App\Helpers\Helpers::can('Creer des works');
...
$path=$file->storeAs(
'storage/uploads', "works"."-".$key."_".time().".".$file->extension()
);
$data[$key]= $path;
}
...
Validator::make($data, [
        ...
        'groupe' => ['cette donnee est obligatoire'],
...
$Works->extra_attributes["extra-data"] = $dat;
...
if(
class_exists('\App\Http\Extras\WorkExtras') &&
method_exists('\App\Http\Extras\WorkExtras', 'beforeSaveCreate')
){
\App\Http\Extras\WorkExtras::beforeSaveCreate($request,$Works);
}

$canSave=true;
if (
class_exists('\App\Http\Extras\WorkExtras') &&
method_exists('\App\Http\Extras\WorkExtras', 'canCreate')
) {
try{
$canSave=\App\Http\Extras\WorkExtras::canCreate($request, $Works);
}catch (\Throwable $e){
...
foreach ($Works->extra_attributes["extra-data"] as $key=>$dat){
$response[$key]=$dat;
}
...
{
try{
$can=\App\Helpers\Helpers::can('Editer des works');
...
$oldCrudData=[];
                ...
                $oldCrudData['libelle']=$Works->libelle;
                $oldCrudData['activite_id']=$Works->activite_id;
                $oldCrudData['user_id']=$Works->user_id;
                                $oldCrudData['debut']=$Works->debut;
                $oldCrudData['fin']=$Works->fin;
                $oldCrudData['groupe']=$Works->groupe;
...
if(
class_exists('\App\Http\Extras\WorkExtras') &&
method_exists('\App\Http\Extras\WorkExtras', 'beforeSaveUpdate')
){
\App\Http\Extras\WorkExtras::beforeSaveUpdate($request,$Works);
}

$canSave=true;
if (
class_exists('\App\Http\Extras\WorkExtras') &&
method_exists('\App\Http\Extras\WorkExtras', 'canUpdate')
) {
try{
$canSave=\App\Http\Extras\WorkExtras::canUpdate($request, $Works);
}catch (\Throwable $e){
...
* @return \Illuminate\Http\Response
*/
public function delete(Request $request, Work $Works)
{
try{
$can=\App\Helpers\Helpers::can('Supprimer des works');
...
if (
class_exists('\App\Http\Extras\WorkExtras') &&
method_exists('\App\Http\Extras\WorkExtras', 'canDelete')
) {
try{
$canSave=\App\Http\Extras\WorkExtras::canDelete($request, $Works);
}catch (\Throwable $e){
...
$actioner=new \App\Http\Actions\WorksActions();
