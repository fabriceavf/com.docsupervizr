...
// use App\Repository\prod\ValidationsRepository;
...
* @param App\Repository\prod\ValidationsRepository $ValidationsRepository
* @param int $id
*/
...
if(
class_exists('\App\Http\Extras\ValidationExtras') &&
method_exists('\App\Http\Extras\ValidationExtras', 'getRelationsApplyWhenUserCallMultipleData')
){
try{
$relationsWhenDataIsMutlipleHide=\App\Http\Extras\ValidationExtras::getRelationsApplyWhenUserCallMultipleData($request);
}catch (\Throwable) {
$relationsWhenDataIsMutlipleHide=[];
}
}
...
if(
class_exists('\App\Http\Extras\ValidationExtras') &&
method_exists('\App\Http\Extras\ValidationExtras', 'filterAgGridQuery')
){
\App\Http\Extras\ValidationExtras::filterAgGridQuery($request,$query);
}


$agGrid=new AgGrid('validations',$query);
$data= $agGrid->getData($request);

if(
class_exists('\App\Http\Extras\ValidationExtras') &&
method_exists('\App\Http\Extras\ValidationExtras', 'AgGridUpdateDataBeforeReturnToUser')
){
$_d=$data['rowData'];
$_d= \App\Http\Extras\ValidationExtras::AgGridUpdateDataBeforeReturnToUser($request,$_d);
$data['rowData'] = $_d;
...
return response()->json(Validation::count());
}
$data = QueryBuilder::for(Validation::class)
->allowedFilters([
...
public function create(Request $request, Validation $Validations)
{


try{
$can=\App\Helpers\Helpers::can('Creer des validations');
...
$path=$file->storeAs(
'storage/uploads', "validations"."-".$key."_".time().".".$file->extension()
);
$data[$key]= $path;
}
...
$Validations->extra_attributes["extra-data"] = $dat;
...
if(
class_exists('\App\Http\Extras\ValidationExtras') &&
method_exists('\App\Http\Extras\ValidationExtras', 'beforeSaveCreate')
){
\App\Http\Extras\ValidationExtras::beforeSaveCreate($request,$Validations);
}

$canSave=true;
if (
class_exists('\App\Http\Extras\ValidationExtras') &&
method_exists('\App\Http\Extras\ValidationExtras', 'canCreate')
) {
try{
$canSave=\App\Http\Extras\ValidationExtras::canCreate($request, $Validations);
}catch (\Throwable $e){
...
$Validations=Validation::find($Validations->id);
$newCrudData=[];
...
foreach ($Validations->extra_attributes["extra-data"] as $key=>$dat){
$response[$key]=$dat;
}
...
public function update(Request $request, Validation $Validations)
{
try{
$can=\App\Helpers\Helpers::can('Editer des validations');
...
$oldCrudData=[];
                ...
                $oldCrudData['libelle']=$Validations->libelle;
...
if(
class_exists('\App\Http\Extras\ValidationExtras') &&
method_exists('\App\Http\Extras\ValidationExtras', 'beforeSaveUpdate')
){
\App\Http\Extras\ValidationExtras::beforeSaveUpdate($request,$Validations);
}

$canSave=true;
if (
class_exists('\App\Http\Extras\ValidationExtras') &&
method_exists('\App\Http\Extras\ValidationExtras', 'canUpdate')
) {
try{
$canSave=\App\Http\Extras\ValidationExtras::canUpdate($request, $Validations);
}catch (\Throwable $e){
...
* @return \Illuminate\Http\Response
*/
public function delete(Request $request, Validation $Validations)
{
try{
$can=\App\Helpers\Helpers::can('Supprimer des validations');
...
if (
class_exists('\App\Http\Extras\ValidationExtras') &&
method_exists('\App\Http\Extras\ValidationExtras', 'canDelete')
) {
try{
$canSave=\App\Http\Extras\ValidationExtras::canDelete($request, $Validations);
}catch (\Throwable $e){
...
$actioner=new \App\Http\Actions\ValidationsActions();
