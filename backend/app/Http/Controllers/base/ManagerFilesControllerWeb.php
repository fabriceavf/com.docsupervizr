...
//            dd($fichiers->extension(),$fichiers->hashName());
            ...
            if (in_array($files->extensions, $imagesExtention)) {
                $img = $files->web_path;

            }
            $data['image_url'] = asset($img);
            $data['download_url'] = route('web_download_files', [$files->id]);
    ...
    public function uploads_files1(Request $request, $id)
    {
...
//        dd($fichier);
        ...
        foreach ($fichiers as $fichier) {
            $result = FilesModel::find($fichier);

            $retour = $result->toArray();
            $imagesExtention = ['jpeg', 'png', 'jpg', 'ico'];
            $img = "images/icons/file-icons/doc.png";
            if (in_array($result->extensions, $imagesExtention)) {
                $img = $result->web_path;

            }
            $retour['image_url'] = asset($img);
            $retour['download_url'] = route('web_download_files', [$result->id]);
...
//            dd($files);
            $new_files[] = FilesModel::find($files)->toArray();
