...
class ListingsControllerWeb extends Controller
{
...
$id="Listings_".Str::uuid()->toString().'_unique';
$id=Str::replace('-','_',$id);
...
'user_id' => Auth::id(),
'action' => 'Navigation vers page listings Reussi',
'ip' =>'Non defini',
'pays' =>'Non defini',
'ville' => 'Non defini',
...
&& !Auth::user()->can("Voir les listings")
){
abort(401);
}
...
$vue = view('/content/Listings.Listings', ['pageConfigs' => $pageConfig, 'menu' => $this->menu,'preselect'=>$new,'options'=>$donnees??[],'options'=>$donnees??[],
...
// La securite qui empeiche darriver sur cette sans avoir une signature valide
if (! $request->hasValidSignature() && !Auth::user()->can("Voir les listings")) {
abort(401);
}
...
$listings_disposition->disposition=$request->get('disposition');
}
...
$vue = view('/content/Listings.listings_component', ['pageConfigs' => $pageConfig, 'menu' => $this->menu,'listings_disposition'=>$listings_disposition,'preselect'=>$new,'options'=>$donnees??[],'options'=>$donnees??[],
]);
return response($vue, 200);
}
/**
...
$vue = view('/content/Listings.Listings', ['pageConfigs' => $pageConfig, 'menu' => $this->menu,'listings_disposition'=>$listings_disposition,'preselect'=>$new,'options'=>$donnees??[],
]);
return response($vue, 200);
}
...
public function index_one(Request $request, Listing $Listings)
{
...
$vue = view('/content/Listings.listings_one', [
...
public function index_one_component(Request $request, Listing $Listings)
{
...
$vue = view('/content/Listings.listings_one_component', [
'pageConfigs' => $pageConfig,
'editorData' => $request->All(),
'Listings' => $Listings,
'listings_disposition'=>$listings_disposition
...
public function show_impression(Request $request, Listing $Listings)
{
// La securite qui empeiche darriver sur cette sans avoir une signature valide
if (! $request->hasValidSignature()) {
abort(401);
...
$vue = view('/content/Listings.impression', [
'pageConfigs' => $pageConfig,
'editorData' => $request->All(),
'Listings' => $Listings,
'listings_disposition'=>$listings_disposition
...
$this->ListingsRepository->show($id);
