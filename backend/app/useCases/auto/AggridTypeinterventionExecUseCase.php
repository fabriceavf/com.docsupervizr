...
class AggridTypeinterventionExecUseCase
{
    ...
    {
        $agGrid = new \App\Http\AgGrid('typeinterventions', $data['__query__']);
        $newData = [];
        $action = 'read';
        $startRow = 0;
        $endRow = 100;
        ...
        try {
            \Illuminate\Support\Facades\DB::table('logs')->insert([
                'user_id' => Auth::id(),
                'action' => 'Lectures des donnees api de  typeinterventions reussi',
                'ip' => 'Non defini',
                'pays' => 'Non defini',
                'ville' => 'Non defini',
