...
class UpdateValidationExecUseCase
{
    ...
    public static function exec($data)
    {
        ...
        $oldValidations = $Validations->replicate();

        $oldCrudData = [];
        $oldCrudData['libelle'] = $oldValidations->libelle;
        $oldCrudData['users'] = $oldValidations->users;
        $oldCrudData['modelslisting_id'] = $oldValidations->modelslisting_id;
        $oldCrudData['creat_by'] = $oldValidations->creat_by;
        $oldCrudData['identifiants_sadge'] = $oldValidations->identifiants_sadge;
        $oldCrudData['nmbvalideurs'] = $oldValidations->nmbvalideurs;
        try {
            $oldCrudData['modelslisting'] = $oldValidations->modelslisting->Selectlabel;
        } catch (\Throwable $e) {
        }
        ...
        DB::table('cruds')->insert(['user_id' => Auth::id(), 'action' => "Update", 'entite' => 'Validations', 'entite_cle' => $Validations->id, 'ancien' => json_encode($oldCrudData), 'nouveau' => json_encode($newCrudData), 'created_at' => now()]);
