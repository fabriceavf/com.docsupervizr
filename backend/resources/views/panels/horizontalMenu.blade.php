...
@endphp
{{-- Horizontal Menu --}}
<div class="horizontal-menu-wrapper row">
  ...
  navbar-shadow menu-border "
                ...
                <li class="nav-item mr-auto">
                    <a class="navbar-brand" href="{{ url('/') }}">
                        <span class="brand-logo">
                            <span style="opacity: 0">22222</span>
                  ...
                  <g id="Artboard" transform="translate(-400.000000, -178.000000)">
                    <g id="Group" transform="translate(400.000000, 178.000000)">
                      ...
                      <polygon id="Path-3" fill="url(#linearGradient-2)" opacity="0.099999994"
                            ...
                            <img class="rounded mx-auto d-block " src="{{ Helper::getLogoName() }}" alt="avatar"
                                 height="40" width="40">
                        </span>
                        {{-- <h2 class="brand-text mb-0">{{ Helper::getAppName() }}</h2> --}}
                    </a>
                </li>
            ...
            <ul class="nav navbar-nav" id="main-menu-navigation" data-menu="menu-navigation">
                ...
                @if (isset($menuData[1]))
                    @foreach ($menuData[1]->menu as $menu)
                        @if(\App\Helpers\Helpers::canShowMenu($menu))
                        @php
                            $custom_classes = '';
                        ...
                        @if (isset($menu->submenu))
                            {{ 'data-menu=dropdown' }}
                            @endif>
                            <a href="{{ Helper::getMenuLink($menu) }}"
                            ...
                            @if (isset($menu->submenu))
                                ...
                                <span>{{ __(Str::ucfirst(Str::lower($menu->name))) }}</span>
                            </a>
                            @if (isset($menu->submenu))
        ...
        <div class="shadow-bottom">
            ...
            {{-- <h6 class="text-dark weight-bold ms-3 mb-0 mt-1">{{Auth::user()->prenom}} {{Auth::user()->nom}}</h6> --}}
            <p class="weight-bold text-primary font-8 mb-0 mt-0">{{Auth::user()->email}}</p>
            <a href="{{\App\Helpers\Helpers::getLogoutRoute()}}">
                <button class="btn btn-danger mx-2  mt-0">Deconnection</button>
            </a>
        </div>
