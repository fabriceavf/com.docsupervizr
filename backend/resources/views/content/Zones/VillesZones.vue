    ...
    <div class="row">
        ...
        <b-modal :id="formId" :size="formWidth">
            <template #modal-title>
                <div v-if="formState == 'Update'">Update Villeszones #{{ formData.id }}</div>
                <div v-if="formState == 'Create'">Create Villeszones</div>
            </template>

            <!-- <div > -->
            <AgGridSearch v-if="formState == 'Create'" :columnDefs="VillecolumnDefs" :extrasData="extrasData"
                          :filterFields="[ 'libelle']" :sizeColumnsToFit="md" :url="Villeurl" filter-key=""
                          filter-value="" @destruction="finishaddVilles">
            </AgGridSearch>
            <!-- </div> -->
...
export default {
    ...
    data() {
        ...
        return {
            ...
            formKey: 0,
            tableKey: 0,
            url: 'http://127.0.0.1:8000/api/villeszones-Aggrid',
            table: 'villeszones',
            zonesData: [],
            requette: 1,
            columnDefs: null,
    ...
    computed: {
        ...
        extrasData: function () {
            let params = {};
            params["zone_id"] = {values: [this.ParentId], filterType: "set"};
            return {
                baseFilter: params,
    ...
    created() {
        this.url = this.axios.defaults.baseURL + '/api/villeszones-Aggrid',
            this.Villeurl = this.axios.defaults.baseURL + '/api/villes-Aggrid',
    ...
    methods: {
        ...
        addvilles(element) {
            ...
            this.form.ville_id = element.id
            this.form.zone_id = this.ParentId
            this.axios.post('/api/villeszones', this.form).then(response => {
                this.isLoading = false;
                this.Villesadd++;
        ...
        DeleteLine(element) {
            this.isLoading = true
            this.axios.post('/api/villeszones/' + element.id + '/delete').then(response => {
                this.isLoading = false
