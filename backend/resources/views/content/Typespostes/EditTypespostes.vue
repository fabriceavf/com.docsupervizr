        ...
        <form @submit.prevent="EditLine()">
            <div class="mb-3">
                ...
                <div class="form-group">
                    ...
                    <input v-model="form.identifiants_sadge" :class="errors.identifiants_sadge?'form-control is-invalid':'form-control'"
                           type="text">
                ...
                <label>Nombre d'agent max </label>
                <div class="row form-group cadre ">

                    <div class="form-group col-sm mt-1">
                        <input :checked="form.maxagent == 1" :value="form.maxagent" class="fakeCheckBox mx-1"
                               type="checkbox" @change="updateCheckboxValue('maxagent')">
                        <label>Activer </label>

                        <div v-if="errors.maxagent" class="invalid-feedback">
                            <template v-for=" error in errors.maxagent"> {{ error[0] }}</template>

                        </div>
                    </div>
                </div>
                <div class="row">
                    <HorairestypespostesView :parentId="form.id" parentKey="poste"></HorairestypespostesView>
                </div>
            </div>
...
import HorairestypespostesView from "../Horairestypespostes/HorairestypespostesView.vue";
import CustomSelect from "@/components/CustomSelect.vue"

export default {
    name: 'EditTypespostes',
    components: {CustomSelect, Files, HorairestypespostesView},
    ...
    methods: {
        ...
        EditLine() {
            this.isLoading = true
            this.axios.post('/api/typespostes/' + this.form.id + '/update', this.form).then(response => {
        ...
        DeleteLine() {
            this.isLoading = true
            this.axios.post('/api/typespostes/' + this.form.id + '/delete').then(response => {
                this.isLoading = false
