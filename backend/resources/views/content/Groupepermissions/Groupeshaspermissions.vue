...
export default {
    name: 'RolesView',
    components: {DataTable, AgGridTable, DataModal, AgGridSearch, AgGridBtnClicked},
    props: ['groupeId'],
    data() {
    ...
    computed: {
        ...
        extrasData: function () {
            let retour = {};
            let params = {};
            params["groupepermission_id"] = {values: [this.groupeId], filterType: "set"};
    ...
    created() {
        this.url = this.axios.defaults.baseURL + '/api/permissions-Aggrid',
    ...
    methods: {
        ...
        addUser(element) {
            let data = {
                id: element.id,
                groupepermission_id: this.groupeId,
            };
            // console.log("voici les donnees ===>", data);

            this.isLoading = true;
            this.axios
                .post('/api/permissions/' + data.id + '/update', data)
                .then((response) => {
                    this.isLoading = false;
                    this.permAdded++;
