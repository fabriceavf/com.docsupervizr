        ...
        <b-modal :id="formId" :size="formWidth">
            <template #modal-title>
                <div v-if="formState == 'Update'">
                    Update Travailleurs #{{ formData.id }}
                </div>
                <div v-if="formState == 'Create'">Create Travailleurs</div>
            </template>

            <EditTravailleurs v-if="formState == 'Update'" :key="formKey" :HorairesData="HorairesData" :data="formData"
            ...
            <AgGridSearch v-if="formState == 'Create'" :columnDefs="addListing.columnDefs"
                          ...
                          :url="addListing.url" filter-key="type_id" filter-value="2,3" @destruction="finishAddUser">
            </AgGridSearch>
...
import CreateTravailleurs from "./CreateTravailleurs.vue";
import EditTravailleurs from "./EditTravailleurs.vue";
...
export default {
    ...
    props: ["parent", "horaireId", "horaires", "typespostes", "tacheId"],
    data() {
        return {
            formId: "Travailleurs",
            formState: "",
            ...
            formKey: 0,
            tableKey: 0,
            url: "http://127.0.0.1:8000/api/travailleurs-Aggrid",
            table: "Travailleurs",
            HorairesData: [],
            usersData: [],
            requette: 2,
    ...
    computed: {
        ...
        extrasData: function () {
            ...
            params["tache_id"] = {values: [this.tacheId], filterType: "set"};
    ...
    created() {
        (this.url = this.axios.defaults.baseURL + "/api/travailleurs-Aggrid"),
    ...
    methods: {
        ...
        DeleteLine(element) {
            this.isLoading = true;
            this.axios
                .post("/api/travailleurs/" + element.id + "/delete")
                .then((response) => {
                    this.isLoading = false;
                    this.tableKey++;
        ...
        present(element, value) {
            ...
            this.isLoading = true;
            this.axios
                .post("/api/travailleurs/" + element.id + "/update", data)
                .then((response) => {
                    this.isLoading = false;
                    this.tableKey++;
        ...
        nonpresent(element, value) {
            ...
            this.isLoading = true;
            this.axios
                .post("/api/travailleurs/" + element.id + "/update", element)
                .then((response) => {
                    this.isLoading = false;
                    this.tableKey++;
                    this.$toast.success("Operation effectuer avec succes");
