        ...
        <form @submit.prevent="EditLine()">
            <div class="mb-3">
                ...
                <div class="form-group">
                    ...
                    <CustomSelect
                        :key="form.interventionuser"
                        :columnDefs="['libelle']"
                        :oldValue="form.interventionuser"
                        :renderCallBack="(data)=>`${data.Selectlabel}`"
                        :selectCallBack="(data)=>form.interventionuser_id=data.id"
                        :url="`${axios.defaults.baseURL}/api/interventionusers-Aggrid`"
                        filter-key=""
                        filter-value=""
                    />
                    <div v-if="errors.interventionuser_id" class="invalid-feedback">
                        <template v-for=" error in errors.interventionuser_id"> {{ error[0] }}</template>
...
export default {
    ...
    methods: {
        ...
        EditLine() {
            this.isLoading = true
            this.axios.post('/api/interventiondetails/' + this.form.id + '/update', this.form).then(response => {
        ...
        DeleteLine() {
            this.isLoading = true
            this.axios.post('/api/interventiondetails/' + this.form.id + '/delete').then(response => {
                this.isLoading = false
